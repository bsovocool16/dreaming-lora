...
This line of work has explicitly scoped itself to within-context operation. Atlas states the scoping directly: memory is maintained within the global context without updating model parameters across independent contexts, and no persistent learning carries over once the memory is cleared.
...
Each consolidation step is bounded not only in rank (directional complexity) but in operator norm (step intensity). This prevents any single experience, however surprising, from producing a deformation larger than the surrounding adapter structure can absorb. The trust region is a safeguard against blowup in the co-constitutive update dynamics analyzed in \S 4: it keeps iterates in a compact set where the bounded-tracking stability target applies.

\subsection{Shadow Adapter (Polyak--Ruppert Averaging)}

A slowly-updated exponential moving average of the adapter state is maintained alongside the live adapter. The live adapter responds to recent experience and may exhibit short-term oscillation; the shadow adapter smooths these fluctuations into a stable trajectory \cite{mou2020}. This decouples tracking (responsiveness to new experience) from estimation (stable representation of long-run adaptation). In the stability analysis of \S 4, the shadow adapter provides damping against rotational dynamics in the update map.
...
Second, an additional objective is required for cross-session symbolic operation. Current transformers exhibit symbolic-like behavior within a context window through attention-based reconstruction of abstractions from context. Consolidating these abstractions into parametric structure that persists across sessions requires something reconstruction alone does not provide: the same content, appearing in different contexts, must produce consistent representations in the persistent memory. Reconstruction rewards local prediction accuracy; it does not reward or enforce cross-context representational consistency. Without an objective that does, a persistent adapter can preserve token-level regularities while losing the type-level structure that makes within-context symbolic behavior possible.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Norm-bounded trust region.} Magnitude control independent of rank. The rank constraint limits directional complexity but not step magnitude; a rank-$r$ matrix can have arbitrarily large operator norm. The trust region caps the operator norm of each update, keeping iterates in a compact set where the bounded-tracking analysis applies.
    \item \textbf{Shadow adapter (Polyak--Ruppert averaging).} Rotational damping. Co-constitutive systems with state-dependent noise can exhibit rotational dynamics analogous to those observed in GAN training \cite{mescheder2018}: an update that is correct on average moves the mean gradient elsewhere, producing limit cycles. Iterate averaging damps these dynamics by improving effective contractivity of the update map \cite{mou2020}.
...
These four safeguards are not redundant. A norm bound prevents blow-up but not rotation. Shadow averaging damps rotation but does not limit directional complexity. Rank limits complexity but not magnitude. Sufficient dream samples ensure estimation quality but do not constrain the update itself. The bounded-tracking framework identifies the joint role of all four: together they produce the conditions under which iterates concentrate around a stable set rather than diverging or cycling.

\subsection{Empirical Validation}

The bounded-tracking thesis is theoretically motivated but requires empirical demonstration. We commit to a toy simulation of the consolidation dynamics under conditions where the architecture's safeguards can be individually ablated. A small-dimension adapter ($d = 8$, $r = 2$) over a few hundred consolidation cycles, on a synthetic dream distribution with known structure, should exhibit the bounded-tracking signature: iterates concentrate around an attractor rather than either converging pointwise or diverging, with dispersion scaling with the stepsize. Ablating each safeguard in turn should produce the predicted failure mode: insufficient dream samples should inflate dispersion, loss of the trust region should permit blow-up under large-surprise inputs, loss of shadow averaging should produce rotational dynamics, and loss of the rank constraint should produce over-consolidation of the gradient landscape. Details and results are reported in \S 8.

The simulation is a targeted test of the stability framework, not a substitute for empirical validation of the architecture as a whole. It establishes that bounded tracking obtains in the simplest case where the framework's assumptions hold; whether it extends to the full deployment on a trained transformer is a separate empirical question.
...
\STATE Salience selection (Eq.~11 using $\mu_\text{struct}$): top-$k$ positions $\mathcal{T}$
\STATE Context generation and symmetry-breaking correction (Eq.~13 using $\tau_\text{sb}$), interpolated with mature-regime weighting (Eq.~16) by $\mu_\text{struct}$: $n'$ chimeric dream sequences $d^{(j)}$
...
\FOR{each $d^{(j)}$}
    \STATE Add structured noise (Eq.~15 using $\mu_\text{struct}$)
    \STATE Compute experiential losses: $L_\text{recon}^{(j)}$ (Eq.~5), $L_\text{cohere}^{(j)}$ (Fisher-discriminant, Eq.~6)
\ENDFOR
...
\STATE Update $\rho$ for next cycle: $\hat{\rho}$ from factor-gradient magnitudes (Eq.~9); $\rho \gets (1 - \alpha_\rho) \rho + \alpha_\rho \hat{\rho}$
...
The framework has explicitly scoped itself to within-context operation. The architecture developed here takes the framework as a starting point and extends it in three directions, with the goal of supporting cross-session memory operation.
...
\textbf{Cross-session symbolic operation requires coherence in attentional bias.} Modern transformers, including those equipped with the Miras framework, exhibit symbolic-like behavior within a context window: tracking entities across long passages, recognizing paraphrases, manipulating formal structures. These abstractions are reconstructed from context at each inference pass through attention and the existing parametric machinery; when the conversation ends, the abstractions are gone, and the next session begins with the model's base capacities plus whatever persistent structure has been consolidated. The reconstruction-family attentional biases that populate the Miras framework---$L_2$, $L_p$, Huber, dot-product---preserve local prediction accuracy but do not produce cross-context representational consistency as a distinct property. A persistent adapter trained only on these biases can encode the same content with different representations across sessions, because reconstruction does not require cross-context consistency. The Fisher-discriminant coherence loss developed in \S 3.4 addresses this: it rewards region-level consistency of same-content representations across the contexts in which they appear, building the type-level structure that within-context symbolic operation already exhibits but which reconstruction-only consolidation does not preserve. The contribution is naturally formalized as an extension of the Miras signature to accommodate cross-context attentional biases.

\textbf{Bounded tracking is the appropriate stability framing for persistent learning systems.} Titans and Atlas demonstrate empirical stability through careful mechanism design---gating, forgetting, momentum, weight decay---without a theoretical account of what stability is meant to mean in this setting. Section 4 develops the bounded-tracking framework from constant-stepsize stochastic approximation with Markovian noise \cite{borkar2008, mou2020}, arguing that the appropriate target for any persistent learning system is not fixed-point convergence but ergodic concentration around stable attractors. The framework applies to Titans, Atlas, Memory Caching, and to the architecture developed here; it provides a language for reasoning about why the gating and forgetting mechanisms these architectures rely on should be expected to work, and what specific failure modes they collectively address. For the cross-session extension, the framework identifies four architectural safeguards---rank, norm, averaging, sample size---that jointly support bounded tracking under the additional dynamical pressure of self-generated dream content.
...
\textbf{Stability simulation.} The bounded-tracking thesis (\S 4) requires direct empirical demonstration. A toy simulation with a small adapter ($d = 8, r = 2$) over a few hundred consolidation cycles on a synthetic dream distribution should exhibit the predicted dispersion-around-attractor signature, with dispersion scaling with the stepsize. Ablating each of the four safeguards in turn should produce the predicted failure mode: insufficient samples should inflate dispersion, removing the trust region should permit blow-up under large-surprise inputs, removing shadow averaging should produce rotational dynamics, and removing the rank constraint should produce over-consolidation. This is the most contained empirical commitment in the program; it tests the stability framework specifically and is targeted at establishing that bounded tracking obtains in the simplest case where the framework's assumptions hold.

\textbf{Cross-session benchmark against an online consolidation baseline.} The central empirical question is whether the offline consolidation mechanism supports cross-session learning that an online consolidation mechanism does not. A controlled comparison would deploy an online checkpointed gradient-based memory baseline---ideally a Titans implementation if a public version becomes available, or a continuously-trained LoRA adapter as a simpler proxy that captures the relevant online-vs-offline contrast---and the Dreaming LoRA architecture on a benchmark requiring retention across separate conversations: structural consistency tasks (does the system maintain the same abstraction of a concept introduced in conversation A when queried in conversation B), accumulation tasks (does the system integrate information presented across multiple conversations rather than only within each one), and cold-start tests (does the system exhibit the path-dependent pathology in initial conversations that Phase 1 curriculum is meant to address). The prediction is that offline consolidation produces measurable advantages on tasks where structural revision matters, while online consolidation may be sufficient where only retention is required. The choice of baseline architecture affects what the comparison demonstrates rigorously but not whether the comparison is meaningful: the contrast that matters is between consolidation regimes, not between specific architectures.

\textbf{Coherence loss validation.} The argument that reconstruction alone produces deductive competence without cross-session symbolic operation is testable. A controlled comparison would train two adapters under identical conditions except for the loss function (reconstruction-only versus composite with Fisher-discriminant coherence) and evaluate whether cross-session paraphrase recognition, entity tracking across separated conversations, and abstract structural reasoning differ. The prediction is that the composite-loss adapter preserves cross-session symbolic operation while the reconstruction-only adapter loses it even when local prediction performance is similar.
...
\textbf{Maturity signal calibration.} The participation-ratio definition of $\mu_\text{struct}$ (\S 5.2) is one defensible choice; whether high-water-mark ratcheting is preferable to windowed integration, what saturation scale $N_\text{sat}$ to use for $\mu_\text{exp}$, and how the Phase 1 exit threshold should be calibrated against $\mu_\text{struct}$ are empirical questions that the simulation and benchmark work can begin to address.
...
\textit{Privacy and adversarial robustness.} A per-user LoRA adapter encodes experiential residue that may be extractable through adversarial probing; the salience weighting can be exploited by deliberately contradictory input. Quality filtering external to the consolidation loop will be necessary for robust deployment.

\textit{Base model migration.} A mechanism for adapter migration across base model versions is needed for any deployment scenario.
