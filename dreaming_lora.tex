...
\end{equation}
where $\lambda_\text{base}$ (default 0.1) is a coherence floor ensuring that some abstraction always occurs regardless of reconstruction pressure---analogous to the biological observation that REM sleep is never zero, even in NREM-dominant early sleep cycles. The composite loss is then:
\begin{equation}
    L_\text{total} = \lambda_\text{recon} \cdot L_\text{recon} + \lambda_\text{cohere} \cdot L_\text{cohere} + L_\text{homeo}.
\end{equation}
...
\end{equation}
where $\hat{\rho}_t$ is the current cycle's estimate and $\alpha_\rho$ is set to match the shadow adapter's EMA rate $\beta$, so that the loss balance and the stable identity evolve on the same timescale.

The composite loss introduces three new hyperparameters: $\tau$ (contrastive temperature), $\eta$ (homeostatic decay rate / timescale of forgetting), and $\lambda_\text{base}$ (coherence floor). All other balance parameters ($\mu$, $\rho$, $\alpha_\rho$) are derived from signals the system already computes.
...
\begin{algorithmic}[1]
\REQUIRE K/V cache $\{(h_t, \ell_t)\}$ from conversation, current LoRA adapter $\theta = (A_Q, B_Q, A_V, B_V)$, base model weights $W$, hyperparameters $k, n, r_{\max}, \delta_{\max}, \alpha, \beta, \gamma, \epsilon_0, \lambda, \tau, \eta, \lambda_\text{base}$
...
\FOR{each selected context $c^{(j)}$}
    ...
    \]
    \STATE Process $d^{(j)}$ through model + adapter $\theta$; compute composite loss $L_j = \lambda_\text{recon} L_\text{recon}^{(j)} + \lambda_\text{cohere} L_\text{cohere}^{(j)} + L_\text{homeo}$ (\S 3.6).
    \STATE Extract gradient $G_j \gets \nabla_{W_Q} L_j$ (repeat analogously for $W_V$).
\ENDFOR
\STATE \textbf{Update loss balance:} Compute $\hat{\rho} \gets \norm{\bar{G}_r}_F / (\norm{\bar{G}_r}_F + \epsilon)$; smooth $\rho \gets (1-\alpha_\rho)\rho + \alpha_\rho \hat{\rho}$. Set $\lambda_\text{recon}, \lambda_\text{cohere}$ via Eq.~(9) for next cycle.
...
The key technical contributions are: (1) the formalization of the consolidation objective as gradient-space SVD with rank as an epistemic parameter; (2) a composite loss with reconstruction, coherence, and homeostatic components whose balance is self-regulated via spectral energy modulated by adapter maturity, corrected by the infant-sleep inversion; (3) the specification of dream content generation as adapter-biased context transplantation with anti-bias correction, governed by a maturity-dependent schedule; (4) the stability analysis via two-timescale SA with bounded tracking as the target regime; (5) the derivation of four disambiguated safeguards from convergence requirements; and (6) a three-phase developmental trajectory derived from the architecture's own feedback dynamics.
