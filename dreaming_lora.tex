...
\FOR{each $d^{(j)}$}
    ...
    \STATE \quad $L_{\rm homeo}^{(j)} \gets \eta (\|A_Q\|_F^2 + \|B_Q\|_F^2 + \|A_V\|_F^2 + \|B_V\|_F^2)$ \quad (Eq.~7)
\ENDFOR
\STATE Reconstruction pressure: $\rho \gets \|\bar{G}_r\|_F / (\|\bar{G}_r\|_F + \epsilon)$ (Eq.~8), EMA-smoothed
\STATE Loss weights: $\lambda_{\rm recon} = \mu\rho / (1+\lambda_{\rm base})$, $\lambda_{\rm cohere} = (1 - \mu\rho + \lambda_{\rm base})/(1+\lambda_{\rm base})$ (Eq.~9)
\STATE Composite loss per dream: $L_j = \lambda_{\rm recon} L_{\rm recon}^{(j)} + \lambda_{\rm cohere} L_{\rm cohere}^{(j)} + L_{\rm homeo}$
...
\STATE Update live $\theta \gets \theta + \alpha \Delta W$; shadow $\theta_{\rm shadow} \gets (1-\beta)\theta_{\rm shadow} + \beta \theta$
\end{algorithmic}
\end{algorithm}
